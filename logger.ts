...
const logger = async (
  ...
  const log_string: string = `Request:: ${format(new Date(Date.now()), "MM-dd-yyyy hh:mm:ss.SSS")}, ${request.ip}, "${request.method}, ${request.url.pathname}", ${String(status)}.
  \rResponse:: ${responseBody}, ${responseTime}\n`;
   status >= 500
